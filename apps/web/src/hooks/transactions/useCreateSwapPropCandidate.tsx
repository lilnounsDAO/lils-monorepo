...
export function useCreateSwapPropCandidate(): UseCreateSwapPropCandidateReturnType {
  ...
  const createCandidate = useCallback(
    (userNoun: Noun, treasuryNoun: Noun, tip: bigint, reason: string) => {
      ...
      const propTitle = `Lilnouns.wtf: Swap Lil Noun ${userNoun.id} for Lil Noun ${treasuryNoun.id}`;
...
This proposal seeks to swap **Lil Noun ${userNoun.id}${
          tip > BigInt(0)
            ? ` + ${formatTokenAmount(tip, NATIVE_ASSET_DECIMALS, 6)} WETH`
            : ""
        }** for **Lil Noun ${treasuryNoun.id}** from the LilNouns DAO treasury.

Lil Noun ${userNoun.id}
...
## This Prop was created using Lilnouns.wtf.

[Lilnouns.wtf](lilnouns.wtf) is a tool built for the Lil Nouns community by [Paperclip Labs](https://paperclip.xyz/). It allows Lil Noun owners to easily create proposals to swap their Lil Noun for a Lil Noun in the DAO treasury.`,
      ];
