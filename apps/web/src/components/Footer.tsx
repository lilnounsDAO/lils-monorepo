...
const SOCIALS: { name: string; icon: string; href: string }[] = [
  { name: "X", icon: "/socials/x.svg", href: "https://x.com/lilnounsdao" },
  {
    name: "Discord",
      ...
      <div className="flex w-full flex-col justify-between gap-20 px-4 pt-14 md:flex-row md:px-10">
        <div className="flex flex-col gap-5">
          <div className="flex items-center gap-3 paragraph-lg">
            ...
            <span className="pt-[4px]">for Lil Nouns DAO</span>
          </div>
          <div className="flex max-w-[448px] flex-col gap-5">
            {/* <span>
              Join our mailing list to stay in the loop with our newest feature
              releases and tips and tricks for navigating Lilnouns.wtf.
            </span>
            <MailingListForm /> */}
          </div>
          <div className="flex gap-4">
            {SOCIALS.map((item) => (
              <LinkExternal
                href={item.href}
                className="flex h-12 w-12 items-center justify-center rounded-full bg-content-primary"
                key={item.name}
              >
            ...
            <div key={i} className="flex flex-col gap-4">
              {items.map((item, j) =>
                item.external ? (
                  ...
                  // Use "to" instead of "href" for react-router-dom's Link
                  <Link
                    to={item.href}
                    key={j}
