...
export function ApproveWeth({ amount, progressStepper }: ApproveNounProps) {
  const { approveErc20, error, status } = useApproveErc20();
  
  // Convert wagmi status to TransactionState
  const txnState = status === "pending" ? "pending-signature" : "idle";
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6">
      ...
      <div className="flex w-full flex-col gap-1">
        <TransactionButton txnState={txnState} onClick={approveCallback} className="w-full">
          Approve WETH
        </TransactionButton>
