...
export default function TreasuryStats({ data }: StatsProps) {
  ...
  // Guard against empty/missing data to avoid runtime crashes
  // if (!data || data.length === 0) {
  //   return (
  //     <div className="paragraph-sm text-semantic-negative">
  ...
  const minTimestamp = Number(data[0]?.timestamp ?? 0);
  ...
  // If filters remove everything, bail out gracefully
  if (!treasuryBalance.length) {
    return (
      <div className="paragraph-sm text-semantic-negative">
        No financial snapshot data in the selected range.
      </div>
    );
  }
  ...
  const revenueVsCostData = data
    .map((d) => ({
      ...
      cost: Number(
        currencySelector == "USD" ? d.propSpendInUsd : d.propSpendInEth
      ),
    }))
    .filter((d) => d.timestamp >= timeCutoff);

  if (!revenueVsCostData.length) {
    return (
      <div className="paragraph-sm text-semantic-negative">
        No revenue/spend data in the selected range.
      </div>
    );
  }
